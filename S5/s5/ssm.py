...
# A_config = aqt_config.fully_quantized(fwd_bits=8, bwd_bits=8)
...
def apply_ssm(
    ...
    Bu_elements = jax.vmap(  # (((inputs.ndim -1,), (0,)), ((), ())),
        lambda u: quant_config.B_quantizer(B_bar, u, dimension_numbers=...)
    )(input_sequence)
    ...
    if conj_sym:
        return jax.vmap(lambda x: 2 * (C_tilde @ x).real)(
            xs
